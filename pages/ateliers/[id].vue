<template>
  <div class="bg-[#F5F7FA] pb-16 pt-8">
    <div class="mx-auto max-w-[1200px] px-4 sm:px-6">
      ...
      <p class="mb-8 max-w-3xl text-sm leading-relaxed text-slate-500 sm:text-base">
        Remplissez le formulaire ci-contre pour vous inscrire à l’atelier de votre choix.
      </p>
          ...
          <div class="rounded-2xl bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.08)] sm:p-6">
            ...
            <form class="space-y-5" @submit.prevent="submitRegister">
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                  <label class="block sm:col-span-1">
                    ...
                    <input
                      ...
                      class="w-full rounded-lg border border-brandBlue/35 bg-white px-3 py-2.5 text-sm text-slate-800 outline-none focus:border-brandBlue focus:ring-2 focus:ring-brandBlue/15"
                      placeholder="Nelly Mbeg"
                    >
                  </label>
                  <label class="block sm:col-span-1">
                    <span class="mb-1.5 block text-sm font-semibold text-brandBlue">
              ...
              <button
                ...
                :disabled="registering || maxPlaces < 1"
              >
                <svg class="h-4 w-4 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
...
async function loadWorkshop() {
  ...
  try {
    workshop.value = await $fetch<WorkshopDetail>(`${config.public.apiBase}/public/ateliers/${workshopId.value}`)
...
async function submitRegister() {
  ...
  try {
    const res = await $fetch<{ code: string; accountKind: 'PARENT' | 'VISITEUR' }>(
      `${config.public.apiBase}/public/ateliers/${workshop.value.id}/register`,
      {
        method: 'POST',
        body: {
    ...
    registerSuccess.value =
      res.accountKind === 'PARENT'
        ? `Inscription enregistrée (${res.code}) sur votre compte parent. Elle est en attente de validation.`
        : `Inscription enregistrée (${res.code}). Elle est en attente de validation.`
    await loadWorkshop()
  } catch (e: unknown) {
    const err = e as { data?: { message?: string | string[] } }
    const raw = err?.data?.message
